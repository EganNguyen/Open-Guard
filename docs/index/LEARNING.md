...
Initially, a global `defaultResolver` was used for mocking in tests. This caused a data race when tests were run with the `-race` flag. 
- **Lesson:** Avoid global state for mockable dependencies. Always prefer local injection via function parameters or struct fields.
- **Fix:** Refactored `NewSafeHTTPClient` to accept an optional `hostResolver` interface.
